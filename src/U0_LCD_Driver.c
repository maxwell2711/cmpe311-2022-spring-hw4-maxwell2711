...
//RWR:
// LCD_WriteDigit:
//   added cast to int in avoid warning about indexing with char
//   modifed all types to be explicitly unsigned
...
void LCD_WriteDigit(char c, char digit)
{
    ...
    if ((c >= '*') && (c <= 'z'))
    {
        ...
        seg = (unsigned int) LCD_character_table[(int)c]; //RWR: added cast to int to avoid warning about indexing with char
    }
